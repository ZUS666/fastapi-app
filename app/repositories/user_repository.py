...
class UserRepository(IUserRepository):
    ...
    async def get_by_email(self, email: EmailStr) -> UserCredentialsSchema | None:
        ...
        return UserCredentialsSchema.model_validate(user, from_attributes=True) if user else None
    ...
    async def update_profile(
        ...
        async with self.session as session:
            profile: Profile = await UserPostgres(session).update_profile(user_id, profile_schema)
